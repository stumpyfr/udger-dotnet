...
namespace Udger.Parser
{
    public class UdgerParser
    {
            ...
            if (!string.IsNullOrEmpty(_ip))
            {
                ...
                if (dt.Connected)
                {
                    ...
                    if (ipVer != 0)
                    {
                        ...
                        var ipTable = dt.Db.Query(@"SELECT udger_crawler_list.id as botid,ip_last_seen,ip_hostname,ip_country,ip_city,ip_country_code,ip_classification,ip_classification_code,
                                          name,ver,ver_major,last_seen,respect_robotstxt,family,family_code,family_homepage,family_icon,vendor,vendor_code,vendor_homepage,crawler_classification,crawler_classification_code,crawler_classification
                                          FROM udger_ip_list
                                          JOIN udger_ip_class ON udger_ip_class.id=udger_ip_list.class_id
                                          LEFT JOIN udger_crawler_list ON udger_crawler_list.id=udger_ip_list.crawler_id
                        ...
                        if (ipTable != null && ipTable.Any())
                        {
                            this.prepareIp(ipTable.FirstOrDefault());
                        }
                        if (ipVer == 4)
                        {
                            ...
                            var dataCenter = dt.Db.Query(@"select name, name_code, homepage
                                       FROM udger_datacenter_range
                                       JOIN udger_datacenter_list ON udger_datacenter_range.datacenter_id = udger_datacenter_list.id
                                       where iplong_from <= " + ipLong.ToString() + " AND iplong_to >=" + ipLong.ToString());

                            if (dataCenter != null && dataCenter.Any())
                            {
                                this.prepareIpDataCenter(dataCenter.FirstOrDefault());
                            }
                        }
        ...
        private void processOS(string uaString, ref int os_id, int clientId)
        {
            ...
            if (rowid != -1)
            {
                ...
                var opSysRs = dt.Db.Query(q);
               // DataTable opSysRs = dt.selectQuery(q);
                this.prepareOs(opSysRs.FirstOrDefault(), ref os_id);
            }
            else if(clientId != 0)
            {
                    //DataTable opSysRs = dt.selectQuery(String.Format(UdgerSqlQuery.SQL_CLIENT_OS, clientId));
                var opSysRs = dt.Db.Query(String.Format(UdgerSqlQuery.SQL_CLIENT_OS, clientId));
                if (opSysRs != null && opSysRs.Any())
                    {
                        this.prepareOs(opSysRs.FirstOrDefault(), ref os_id);
                    }
        ...
        private void processClient(string uaString, ref int os_id, ref int clientId, ref int classId)
        {
            ...
            var userAgentRs = dt.Db.Query(q);
            if (userAgentRs != null && userAgentRs.Any())
            {

                this.prepareUa(userAgentRs.FirstOrDefault(), true, ref clientId, ref classId);
                classId = 99;
                clientId = -1;
            }
            else {
                ...
                if (rowid != -1)
                {
                    ...
                    userAgentRs = dt.Db.Query(String.Format(UdgerSqlQuery.SQL_CLIENT, rowid));
                    this.prepareUa(userAgentRs.FirstOrDefault(), false, ref clientId, ref classId);
                    //patchVersions(ret);
                }
                else {
                    userAgent.UaClass = "Unrecognized";
        ...
        private void processDevice(string uaString, ref int classId)
        {
            ...
            if (rowid != -1)
            {
                ...
                var devRs = dt.Db.Query(String.Format(UdgerSqlQuery.SQL_DEVICE, rowid));
                this.prepareDevice(devRs.FirstOrDefault(), ref classId);
            }
            else {
                if ( classId != -1)
                {
                    //DataTable devRs = dt.selectQuery(String.Format(UdgerSqlQuery.SQL_CLIENT_CLASS, classId.ToString()));
                    var devRs = dt.Db.Query(String.Format(UdgerSqlQuery.SQL_CLIENT_CLASS, classId.ToString()));
                    if (devRs != null && devRs.Any())
                    {
                        this.prepareDevice(devRs.FirstOrDefault(), ref classId);
                    }
                }
        ...
        private void processDeviceBrand()
        {
           ...
           // DataTable devRs = dt.selectQuery(String.Format(UdgerSqlQuery.SQL_DEVICE_REGEX,this.userAgent.OsFamilyCode,this.userAgent.OsCode));
            var devRs = dt.Db.Query(String.Format(UdgerSqlQuery.SQL_DEVICE_REGEX,this.userAgent.OsFamilyCode,this.userAgent.OsCode));
            if (devRs != null && devRs.Any())
            {
                foreach (var row in devRs)
                {
                    ...
                    if (devId != null && regex != null)
                    {
                        ...
                        if (reg.IsMatch(this.ua))
                        {
                            ...
                            var devNameListRs = dt.Db.Query(String.Format(UdgerSqlQuery.SQL_DEVICE_NAME_LIST, devId, foo));
                            if (devNameListRs != null && devNameListRs.Any())
                            {
                                var r = devNameListRs.FirstOrDefault();
        ...
        private void prepareUa(dynamic _row,Boolean crawler,ref int clientId, ref int classId)
        {
        ...
        private void prepareOs(dynamic _row, ref int _osId)
        {
        ...
        private void prepareDevice(dynamic _row, ref int _deviceClassId)
        {
        ...
        private void prepareIp(dynamic _row)
        {
        ...
        private void prepareIpDataCenter(dynamic _row)
        {
        ...
        private static WordDetector createWordDetector(DataReader connection, String regexTableName, String wordTableName)
        {
            ...
            var dt = connection.Db.Query("SELECT * FROM " + wordTableName);
            if (dt != null)
            {
    ...
    private static void addUsedWords(HashSet<int> usedWords, DataReader connection, String regexTableName, String wordIdColumn) 
    {
            ...
            var rs = connection.Db.Query("SELECT " + wordIdColumn + " FROM " + regexTableName);
            if (rs != null)
            {
                foreach (var row in rs)
                {
                    usedWords.Add(UdgerParser.ConvertToInt(row.word_id));
                }
            }
    }
    ...
    private static List<IdRegString> prepareRegexpStruct(DataReader connection, String regexpTableName) 
    {
        List<IdRegString> ret = new List<IdRegString>();
            var rs = connection.Db.Query<dynamic>("SELECT rowid, regstring, word_id, word2_id FROM " + regexpTableName + " ORDER BY sequence");
